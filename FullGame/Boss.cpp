...
Boss::Boss(float _x, float _y) :Enemy(_x, _y, ID_BOSSBALL)
{
...
{
	if (__instance == NULL) __instance = new Boss(1220, 100);
	return __instance;
}

void Boss::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
	if (this->DistanceTo(Simon::GetInstance()) <= 300)
		this->SetState(BOSS_STATE_FLY);
	...
	if (dt_die == 0)
	{
		...
		else
		{
			...
			if (GetTickCount() - start_attack >= BOSS_TIME_ATTACK && start_fly == 0)
			{
				...
				vx = vy = 0;
				SmallBall* smallball = new SmallBall(x + BOSS_BBOX_WIDTH / 2, y + BOSS_BBOX_HEIGHT / 2, nx);
				if (s_x == x)
				{
...
void Boss::AutoFly(float next_x, float next_y)
{
	...
	x2 = next_x;
	y2 = next_y;
	Simon::GetInstance()->GetPosition(x1, y1);
...
void Boss::SetFly()
{
	...
	if (_type == 2)
		...
		type = 0;
	float c_x = 100.0, c_y = 0.0;
	//CGame::GetInstance();//->GetCam(c_x, c_y);
