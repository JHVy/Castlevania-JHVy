...
bool Ghost::isStart = false;

Ghost::Ghost(float _x, float _y, int id) :Enemy(_x, _y, id)
{
	this->_type = eType::GHOST;
	animations.clear();
	AddAnimation(1000);
	AddAnimation(800);
	AddAnimation(802);
...
void Ghost::CollisionWithBrick(DWORD dt, LPGAMEOBJECT& obj, float min_tx0, float min_ty0, int nx0, int ny0)
{
...
void Ghost::CollisionWithHiden(DWORD dt, LPGAMEOBJECT& obj, float min_tx0, float min_ty0, int nx0, int ny0)
{
	vector<LPCOLLISIONEVENT> coEvents;
	vector<LPCOLLISIONEVENT> coEventsResult;
